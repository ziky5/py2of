...
def test_create_of_object() -> None:
    ofobject = OfObject(
        objectName="genericObjectName", internalData={"zone": [1, 2, 3, 4, 5]}
    )
...
def test_print_of_object_different_class() -> None:
    ofobject = OfObject(
        objectName="genericObjectName",
        internalData={"zone": [1, 2, 3, 4, 5]},
        className=FieldType.ScalarField,
    )
...
def test_assert_object_name() -> None:
    with pytest.raises(AssertionError):
        ofobject = OfObject(objectName=1, internalData={"zone": [1, 2, 3, 4, 5]})


def test_assert_data_dict() -> None:
    with pytest.raises(AssertionError):
        ofobject = OfObject(objectName=1, internalData=[1, 2, 3, 4, 5])


def test_assert_data_key_str() -> None:
    with pytest.raises(AssertionError):
        ofobject = OfObject(
            objectName="genericObjectName", internalData={1: [1, 2, 3, 4, 5]}
        )


def test_assert_data_item_sequence() -> None:
    with pytest.raises(AssertionError):
        ofobject = OfObject(objectName="genericObjectName", internalData={"zone": 1})
